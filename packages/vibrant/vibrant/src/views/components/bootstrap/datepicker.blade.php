 ...
 * @managed
 *
 * @param string $name Input name|e=datePickerInput
 * @param string $id Input Id, if empty it takes tha same value as $name|o|
 * @param string $label Input label|o|e=Pick a Date
 * @param string $label_class Class to modify input label style|o
 * @param string $placeholder Placeholder for the input|o|e=Pick a date
 ...
 * @param string $prepend Prepend text or icon to this input|o|e=<i class="icon md-calendar"></i>
 ...
 * @param string $attributes Adds the given attributes to the input |o|e=data-date-format=yyyy-mm-dd
 * @param string $class Custom class for this component|o|
 */
if(!isset($prepend)){
    $prepend ='<i class="icon md-calendar"></i>';
}
if(!isset($attributes)){
...
$attributes .= ' data-classes=vib-datepicker data-language='.app()->getLocale();
?>

@bring('jq_airDatepicker@2.2.3')

@include('vibComponent::bootstrap.input', [
    'type' => 'text',
    'plugin' => 'airDatepicker',
    'prepend' => $prepend,
